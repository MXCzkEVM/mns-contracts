...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const resolver = await providerWithEns.getResolver('mxc')
  if (resolver === null) {
    console.log(
      `No resolver set for .mxc; not setting interface ${interfaceId} for NameWrapper`,
    )
    return
  }
  ...
  const tx3 = await resolverContract.setInterface(
    ethers.utils.namehash('mxc'),
    interfaceId,
    nameWrapper.address,
  )
  console.log(
    `Setting NameWrapper interface ID ${interfaceId} on .mxc resolver (tx: ${tx3.hash})...`,
  )
  await tx3.wait()
}
