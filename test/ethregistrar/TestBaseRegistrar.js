...
contract('BaseRegistrar', function (accounts) {
  ...
  before(async () => {
    ...
    registrar = await BaseRegistrar.new(ens.address, namehash.hash('mxc'), {
      from: ownerAccount,
    })
    await registrar.addController(controllerAccount, { from: ownerAccount })
    await ens.setSubnodeOwner('0x0', sha3('mxc'), registrar.address)
  })
  ...
  it('should allow new registrations', async () => {
    ...
    assert.equal(
      await ens.owner(namehash.hash('newname.mxc')),
      registrantAccount,
    )
  ...
  it('should allow registrations without updating the registry', async () => {
    ...
    var block = await web3.eth.getBlock(tx.receipt.blockHash)
    assert.equal(await ens.owner(namehash.hash('silentname.mxc')), ZERO_ADDRESS)
  ...
  it('should permit the owner to reclaim a name', async () => {
    await ens.setSubnodeOwner(ZERO_HASH, sha3('mxc'), accounts[0])
    await ens.setSubnodeOwner(
      namehash.hash('mxc'),
      sha3('newname'),
      ZERO_ADDRESS,
    )
    assert.equal(await ens.owner(namehash.hash('newname.mxc')), ZERO_ADDRESS)
    await ens.setSubnodeOwner(ZERO_HASH, sha3('mxc'), registrar.address)
  ...
  it('should allow the owner to set a resolver address', async () => {
    ...
    assert.equal(await ens.resolver(namehash.hash('mxc')), accounts[1])
  })
})
