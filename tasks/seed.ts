...
task('seed', 'Creates test subbdomains and wraps them with Namewrapper')
  ...
  .setAction(async ({ name }, hre) => {
    try {
      ...
      console.log(
        `Wrapped NFT for sub2.${name}.mxc is available at ${getOpenSeaUrl(
          ethers,
          NameWrapper.address,
          namehash(`sub2.${name}.mxc`),
        )}`,
      ...
      await (
        await NameWrapper.setFuses(namehash(`${name}.mxc`), CANNOT_UNWRAP, {
          gasLimit: 10000000,
        })
      ).wait()
      ...
      console.log(
        'NameWrapper set CANNOT_SET_RESOLVER fuse successful for sub2',
      )
